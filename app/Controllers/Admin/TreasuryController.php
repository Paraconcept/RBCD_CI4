...
class TreasuryController extends BaseController
{
    public function index(): string
    {
        ...
        $stats = [
            ...
            'f1Total'    => 0, 'f1Paid'   => 0,
            'f2Total'    => 0, 'f2Paid'   => 0,
            'supporters' => 0, 'supportersActive' => 0, 'cardsSold' => 0,
        ];
        ...
        foreach ($rows as $r) {
            ...
            if ($r->is_supporter) {
                $stats['supporters']++;
                if ($r->card && $r->card->status === 'active') $stats['supportersActive']++;
            }
            $stats['cardsSold'] += $r->cardsYear;
        }
    ...
    public function export()
    {
        ...
        $headers = [
            ...
            'Sympathisant', "Cartes achetées {$year}", 'Dernière carte N°', 'Achat', 'Expiration', 'Séances', 'Statut',
        ];
        $sheet->fromArray($headers, null, 'A1');

        $yn  = fn($v) => $v ? 'Oui' : 'Non';
        $statusLabels = ['active' => 'Active', 'complete' => 'Complète', 'expired' => 'Expirée'];
        $row = 2;
        foreach ($rows as $r) {
            $sheet->fromArray([
                ...
                $yn($r->is_supporter), $r->cardsYear,
                $r->card->card_number ?? '', $r->card->purchase_date ?? '', $r->card->expiry_date ?? '',
                $r->card ? $r->card->used . '/' . SupporterCardModel::SESSIONS_PER_CARD : '',
                $r->card ? $statusLabels[$r->card->status] : '',
            ], null, 'A' . $row);
            $row++;
        }
    ...
    {
        $rows = \Config\Database::connect()->table('members m')
            ->select([
                'm.id',
                'm.first_name',
                'm.last_name',
        ...
        $cards = (new SupporterCardModel())->getSummaryByMember($year);
        foreach ($rows as $r) {
            $r->card      = $cards[$r->id]->latest ?? null;
            $r->cardsYear = $cards[$r->id]->countYear ?? 0;
        }
        return $rows;
    }
    ...
    public function saveSettings()
    {
        ...
        $data = [
            ...
            'supporter_card_price' => (float) str_replace(',', '.', $this->request->getPost('supporter_card_price')),
