...
<div class="d-flex align-items-center mb-3 flex-wrap" style="gap:.5rem">
    ...
    <a href="<?= base_url('admin/treasury/revenues/create') ?>" class="btn btn-primary btn-sm">
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-arrow-up mr-2"></i>Recettes <?= $year ?></h3>
    </div>
    <div class="card-body p-0">
        <?php if (empty($rows)): ?>
            <div class="p-3 text-muted"><i class="fas fa-info-circle mr-1"></i>Aucune recette enregistrée pour <?= $year ?>.</div>
        <?php else: ?>
        <table class="table table-sm table-hover mb-0" id="revenuesTable">
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-nowrap" data-order="<?= esc($r->revenue_date) ?>"><?= date('d/m/Y', strtotime($r->revenue_date)) ?></td>
                    ...
                    <td class="text-center text-nowrap">
                        ...
                        <form id="del-<?= $r->id ?>" method="POST"
                              action="<?= base_url('admin/treasury/revenues/' . $r->id . '/delete') ?>" class="d-none">
                            <?= csrf_field() ?>
                        </form>
                    </td>
                </tr>
            ...
            <tfoot class="tfoot-total">
                <tr>
                    ...
                    <td class="text-right font-weight-bold text-success"><?= number_format($total, 2, ',', '.') ?> €</td>
                    <td colspan="3"></td>
                </tr>
            </tfoot>
        </table>
